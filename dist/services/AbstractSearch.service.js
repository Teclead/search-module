...
class AbstractSearchService {
    ...
    setUpDefaultAPI() {
        if (this.options.express && this.options.express.app) {
            this.options.express.app.get(`${this.options.express.apiPath}`, (req, res) => {
    ...
    getServerData() {
        return __awaiter(this, void 0, void 0, function* () {
            let data;
            try {
                data = yield (yield fetch(this.getSearchUrl())).json();
            }
            catch (e) {
                console.warn(e);
            }
            ...
            if (isValidResponse) {
                this.rawData = [];
                data.pathList.forEach((startChild) => this.setUpData(startChild));
    ...
    getSearchResult(search) {
        ...
        if (firstLevelSearch.foundItems === 0 && hasMultipleWords) {
            ...
            const secondSearchTerm = search.split(' ').sort((a, b) => b.length - a.length);
     ...
     */
    getFilteredContent(contentChild, pageContentKeys) {
        const { _jcrContent } = contentChild;
        const content = {};
     ...
     */
    getSynonyms(search) {
        let synonyms = [search.toLowerCase()];
     ...
     */
    getSearchRank(searchModel, searchWithSynonyms) {
     ...
     */
    isKeyWordFoundInSynonym(searchModel, searchWithSynonyms) {
        ...
        for (const search of searchWithSynonyms) {
            try {
                const found = isArray ? (searchModel.searchElement.findIndex(e => (e || '').toLowerCase().includes(search)) > -1)
                    : searchModel.searchElement.toLowerCase().includes(search);
                if (found) {
                    isFound = true;
     ...
     */
    getDefaultSynonyms() {
        const rawText = fs_1.readFileSync(path.resolve(__dirname, '../../lib/synonyms.txt'), 'utf-8').toString();
...
//# sourceMappingURL=AbstractSearch.service.js.map